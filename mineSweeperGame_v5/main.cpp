 ...
 * Purpose: Version 5 of Mine Sweeper game, revised win condition function
 */
...
bool checWin(const char[16][16],const int[16][16],short); //Checks for and returns win condition
...
int main(int argc, char** argv) {
    ...
    while(winCond==0){
        ...
        if(checWin(board,boardm,flags)){
            winCond=1;
            break; //Game has been won by placing all flags or opening all squares
        }
    ...
    if(winCond==1){
        ...
        cout<<"Solved in "<<(finTime/60)<<" mins "<<(finTime%60)<<" sec\n";
